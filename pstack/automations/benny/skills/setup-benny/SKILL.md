...
The human enters setup by pointing Cursor at the pack's `FOR_AGENTS.md`. The bootstrap flow copies the whole pack into the target repository, then reads this file directly at `.claude/automations/benny/skills/setup-benny/SKILL.md`.
...
Add pstack to the target repository's `.claude/settings.json`. If the file or `.cursor` directory does not exist, create it.
