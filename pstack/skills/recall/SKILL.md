...
Transcripts live at `~/.claude/projects/<slug>/<uuid>.jsonl`, where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-" (so `/Users/you/proj` becomes `Users-you-proj`; on Windows the drive colon collapses as well, so `D:\repos\proj` becomes `D--repos-proj`). Every line is one chat message.
